...
# Input:
# - A string of commands and values for the mini-language to complete

# Output:
# - Integers as the program language commands
...
# - All operations are integer oprations
...
def minilang(str)
  ...
  str.split.each do |command|
    ...
    when "ADD" then reg = add(stack, reg)
    when "SUB" then reg = sub(stack, reg)
    when "MULT" then reg = mult(stack, reg)
    when "DIV" then reg = div(stack, reg)
    when "MOD" then reg = mod(stack, reg)
    when "POP" then reg = stack.pop
    when "PRINT" then puts reg
    else reg = command.to_i if command.to_i != 0
    end
  end
